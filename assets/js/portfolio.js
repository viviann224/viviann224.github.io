...
function makeModal(){
  for(let projNum=0; projNum<project.length; projNum++){
    ...
    for(let i=0;i<project[projNum].img.length;i++){
      ...
      img.setAttribute("src","./assets/img/"+project[projNum].img[i]+"");
    ...
    d11.append(d12)
     ...
     ul.appendChild(li3);
      ...
      if(project[projNum].git){
        ...
        btn3.setAttribute("class","hoverbtn btn-secondary info hovereffect border ");
...
function createProject(){
  for(let projNum=0;projNum<project.length;projNum++){
  	...
  	img.setAttribute("src","./assets/img/"+project[projNum].img[0]+"");
